    ...
    SubShader {
        ...
        Pass {
            ...
            UNITY_INSTANCING_BUFFER_START( Props )
                ...
                UNITY_DEFINE_INSTANCED_PROP( float, _HoleSize)
                UNITY_DEFINE_INSTANCED_PROP( float, _HoleEdgeSmoothness)
...
////// Lighting:
////// Emissive:
                float _HoleSize_var = UNITY_ACCESS_INSTANCED_PROP( Props, _HoleSize );
                float node_9892 = (_HoleSize_var*-1.0+1.0);
                float _HoleEdgeSmoothness_var = UNITY_ACCESS_INSTANCED_PROP( Props, _HoleEdgeSmoothness ); // Maybe this can be controlled by distance?
                float _DistortionStrength_var = UNITY_ACCESS_INSTANCED_PROP( Props, _DistortionStrength );
                float3 emissive = (smoothstep( (node_9892+_HoleEdgeSmoothness_var), (node_9892-_HoleEdgeSmoothness_var), (1.0 - pow(1.0-max(0,dot(normalDirection, viewDirection)),0.15)) )*tex2D( _GrabTexture, ((pow((1.0 - pow(1.0-max(0,dot(normalDirection, viewDirection)),_DistortionStrength_var)),6.0)*(sceneUVs.rg*-2.0+1.0))+sceneUVs.rg)).rgb);
