...
public class CoinSpawner : MonoBehaviour
{
    ...
    private void Update()
    {
        if (this.GetComponent<Collider2D>().IsTouching(playerCollider))
        {
            ...
            if (_timer <= 0)
            {
                Instantiate(coin, new Vector3(hatCoinSpawner.position.x, hatCoinSpawner.position.y + 1, hatCoinSpawner.position.z), Quaternion.identity)
                .GetComponent<Rigidbody2D>().AddForce(new Vector2(Random.Range(minRandomRange, maxRandomRange), Random.Range(minRandomRange, maxRandomRange)), ForceMode2D.Force);
                _timer = timerNum;
            }
        }
    }
